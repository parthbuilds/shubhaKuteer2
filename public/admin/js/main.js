...
document.addEventListener("DOMContentLoaded", () => {
  // Try to fetch admin info from localStorage (saved at login)
  const adminData = localStorage.getItem("admin");

  if (adminData) {
    const admin = JSON.parse(adminData);

    // Update the dropdown with admin details
    const userNameElement = document.querySelector(
      "#dropdownMenuButton3 .body-title"
    );
    const userRoleElement = document.querySelector(
      "#dropdownMenuButton3 .text-tiny"
    );
    const userAvatarElement = document.querySelector(
      "#dropdownMenuButton3 .image img"
    );

    if (userNameElement) userNameElement.textContent = admin.name;
    if (userRoleElement) userRoleElement.textContent = admin.role || "Admin";
    if (userAvatarElement) {
      userAvatarElement.src = admin.avatar || "images/avatar/user-1.png";
    }
  }
});
